...
%         end
        ...
        ee=strfind(out{1,1}(i,:),'DispBeamColumn3d'); % look for DispBeamColumn3d
        if isempty(ee{1,1})==0
            %fprintf('found DispBeamColumn3d at line: %d\n',i);
            temp22=textscan(out{1}{i},'DispBeamColumn3d, element id:  %d');
            temp33=textscan(out{1}{i+1},'Connected external nodes:  %d %d'); 
            ele = [ele;temp22{1}];
            iNd=[iNd; temp33{1}];
            jNd=[jNd; temp33{2}]; 
            Typ{end+1,1}='DispBeamColumn3d';
        end
